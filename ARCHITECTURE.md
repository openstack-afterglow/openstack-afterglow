...
- Dev Compose의 Palimpsest API/bootstrap/worker는 sibling repository 전체가 아니라 package root `../palimpsest/hub`와 그 안의 `Dockerfile`을 build context로 사용한다. Runner preflight도 같은 파일을 검사한다. 이는 현행 Hub package layout과 Dockerfile의 package-local `COPY src` 경계를 맞추는 경로 수정이며 service/image/runtime ownership은 바꾸지 않는다.
...
{
  "schema_version": 1,
  "source_sha256": "8a089b9c00aa9250c253c212646768ec53d99beee5cc841bf444e5713095af25",
  "reviewed_at": "2026-09-21T09:02:55Z",
  "summary": "Reviewed the combined Cloud Shell, chat and Gateway, Waygate and topology, branding and deployment, and VM bootstrap changes; preserved service boundaries, scoped retained layer artifacts to resolved libraries, and moved GPU monitoring to NVIDIA packages."
}
```

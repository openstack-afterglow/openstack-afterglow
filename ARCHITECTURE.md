...
| Lumen chat BFF와 SSE 전달 | `partial` | `test-defined` | 실행·provider·secret·durable journal과 공개 ID→내부 route 해석, 사용자/키 쿼터 판정은 Lumen 소유이며, Afterglow는 `api_model_name`/`api_provider` projection과 쿼터·한도 편집 화면을 제공할 뿐 자체 agent runtime·provider router·accounting이 아니다. 채팅 설정은 채팅 화면 위의 `ChatSettingsOverlay`이고 `/dashboard/chat/settings?section=…` deep link도 채팅과 지정 탭 팝업을 연다. 관리자 사용자 쿼터는 `/admin/chat/quotas`에서 범용 `/api/v1/chat/{path}` proxy(PATCH/PUT 포함)로 Lumen `/v1`에 전달된다. | [`backend/app/api/lumen/`](backend/app/api/lumen/), [`frontend/src/lib/components/chat/ChatSettingsOverlay.svelte`](frontend/src/lib/components/chat/ChatSettingsOverlay.svelte), [`frontend/src/routes/admin/chat/quotas/+page.svelte`](frontend/src/routes/admin/chat/quotas/+page.svelte), [`backend/tests/contracts/test_lumen_proxy.py`](backend/tests/contracts/test_lumen_proxy.py) |
...
| [`frontend/src/lib/api/client.ts`](frontend/src/lib/api/client.ts) `api`, `tryRefresh`, `request` | API base, Authorization, refresh 직렬화, 401/403 처리, prefetch/invalidation | UI → FastAPI |
...
| [`frontend/src/lib/components/chat/ChatPanel.svelte`](frontend/src/lib/components/chat/ChatPanel.svelte), [`frontend/src/lib/components/chat/ChatInput.svelte`](frontend/src/lib/components/chat/ChatInput.svelte), [`frontend/src/lib/components/chat/ModelPickerOverlay.svelte`](frontend/src/lib/components/chat/ModelPickerOverlay.svelte), [`frontend/src/lib/components/chat/ChatSettingsOverlay.svelte`](frontend/src/lib/components/chat/ChatSettingsOverlay.svelte), [`frontend/src/lib/components/chat/ChatApiKeysManager.svelte`](frontend/src/lib/components/chat/ChatApiKeysManager.svelte) | Lumen durable SSE의 bounded 표시, 즉시 history/title metadata reconciliation, context 예산·압축 run 제어, 공개 모델 metadata와 SDK 연결 안내의 반응형 projection, 채팅 설정 팝업과 `?section=` deep-link 탭 선택, API 키 rename·월/주간 한도 편집 | UI → `/api/v1/chat` BFF; 제목·요약 checkpoint·provider/tool 실행·accounting·쿼터 판정은 Lumen 소유 |
| [`frontend/src/routes/admin/chat/quotas/+page.svelte`](frontend/src/routes/admin/chat/quotas/+page.svelte), [`frontend/src/lib/api/chatQuotas.ts`](frontend/src/lib/api/chatQuotas.ts) | Keystone 사용자 목록과 Lumen 사용자 지갑 쿼터(`/api/v1/chat/admin/quotas`)를 `user_id`로 병합해 월·주간 크레딧 한도를 편집; 지갑 없는 사용자는 Lumen이 보고한 기본 월 한도로 표시 | 관리자 UI → `/api/v1/admin/users` + `/api/v1/chat/admin/quotas` proxy |
...
3. `frontend/src/lib/api/client.ts`는 만료 임박 요청을 coalesce하여 `/api/v1/auth/refresh`를 한 번만 호출하고, terminal 401이면 auth를 비우고 `/login`으로 이동한다. logout은 refresh/회전 경쟁을 막는 revocation fence를 사용한다.
...
{
  "schema_version": 1,
  "source_sha256": "2f9ecae6e32e8e47debea6f9b3da3abb4f172496309f9e5b5e61d4dc3870e6bd",
  "reviewed_at": "2026-09-11T08:12:09Z",
  "summary": "No structure impact: Kolla now checks the versioned Keystone public catalog endpoint independently from every Afterglow controller before deployment."
}
```

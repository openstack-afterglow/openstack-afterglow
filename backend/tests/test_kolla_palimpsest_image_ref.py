...
_VALIDATOR_PATH = (
    Path(__file__).resolve().parents[2] / "deploy/kolla/ansible/roles/palimpsest/files/validate_image_ref.py"
)
...
def test_rejects_latest_remote_tag() -> None:
    with pytest.raises(ValueError, match="latest tag is prohibited"):
        _VALIDATOR.validate_image_ref(

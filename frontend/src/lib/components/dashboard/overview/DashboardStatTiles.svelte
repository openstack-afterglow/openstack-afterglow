...
<div class="grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-line bg-line lg:grid-cols-4">
	...
	{:else}
		...
		>
			...
			{/snippet}
			{#snippet footer()}
				{#if summaryError}<span class="text-xs text-[var(--color-state-danger)]">갱신 실패</span>{/if}
			{/snippet}
		</StatTile>
	{/if}
	...
	{:else}
		...
		>
			...
			{/snippet}
			{#snippet footer()}
				{#if quotasError}<span class="text-xs text-[var(--color-state-danger)]">갱신 실패</span>{/if}
			{/snippet}
		</StatTile>
	{/if}
	...
	{:else}
		...
		>
			...
			{/snippet}
			{#snippet footer()}
				{#if k3sError}<span class="text-xs text-[var(--color-state-danger)]">갱신 실패</span>{/if}
			{/snippet}
		</StatTile>
	{/if}
</div>

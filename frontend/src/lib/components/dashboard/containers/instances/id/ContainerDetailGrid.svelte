...
<div class="grid grid-cols-2 gap-4 mb-6">
  <div class="bg-surface-base border border-line rounded-xl p-4">
    ...
    {#if container.status_reason}
      <p class="text-xs text-ink-2 mt-2">{container.status_reason}</p>
    {/if}
  </div>
  <div class="bg-surface-base border border-line rounded-xl p-4">
    <div class="text-xs text-ink-2 mb-1">이미지</div>
    <div class="text-ink-0 text-xs font-mono">{container.image ?? '-'}</div>
  </div>
  <div class="bg-surface-base border border-line rounded-xl p-4">
    <div class="text-xs text-ink-2 mb-1">리소스</div>
...
{#if container.command}
  <div class="bg-surface-base border border-line rounded-xl p-4 mb-4">
    <div class="text-xs text-ink-2 mb-2">명령</div>

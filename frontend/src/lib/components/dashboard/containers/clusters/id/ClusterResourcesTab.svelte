...
{:else if resources.length === 0}
  <div class="text-ink-2 text-sm">스택 리소스 정보를 불러올 수 없습니다</div>
{:else}
      ...
      <tbody>
        {#each resources as r}
          <tr class="border-b border-line/50 text-xs">
            ...
            <td class="py-2 pr-4 text-ink-2 font-mono text-xs">{r.physical_resource_id?.slice(0, 12) || '-'}</td>
            <td class="py-2 pr-4">
            ...
            <td class="py-2 text-ink-2">{r.created_at?.slice(0, 16).replace('T', ' ') ?? '-'}</td>
          </tr>
        {/each}
      </tbody>
    </table>

...
<div class="flex items-start justify-between mb-8">
	...
	<button onclick={onDelete} disabled={saving} class="text-red-400 hover:text-red-300 disabled:text-ink-3 text-sm px-3 py-1.5 rounded border border-red-900 hover:border-red-700 disabled:border-line-2 transition-colors">{isProtected ? '강제 삭제' : '삭제'}</button>
</div>

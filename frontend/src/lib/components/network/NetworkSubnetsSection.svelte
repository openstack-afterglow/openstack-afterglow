...
{#if s.network!.subnet_details.length > 0}
	<div class="bg-surface-base border border-line rounded-xl p-4">
		<h3 class="text-xs text-ink-2 uppercase tracking-wide mb-3">서브넷 ({s.network!.subnet_details.length})</h3>
		<div class="space-y-3">
				...
				<div class="border-b border-line/50 pb-2 last:border-0 last:pb-0">
					...
					<dl class="mt-1 space-y-1 text-xs">
						<div class="flex justify-between">
							<dt class="text-ink-2">CIDR</dt>
							<dd class="text-ink-2 font-mono">{subnet.cidr}</dd>
						</div>
						<div class="flex justify-between">
							<dt class="text-ink-2">게이트웨이</dt>
							<dd class="text-ink-2 font-mono">{subnet.gateway_ip || '-'}</dd>
						</div>
						<div class="flex justify-between">
							<dt class="text-ink-2">DHCP</dt>
							<dd class="{subnet.dhcp_enabled ? 'text-green-400' : 'text-ink-2'}">{subnet.dhcp_enabled ? '활성' : '비활성'}</dd>
						</div>
					</dl>
				</div>
			{/each}
...
{:else}
	<div class="bg-surface-base border border-line rounded-xl p-4">
		<h3 class="text-xs text-ink-2 uppercase tracking-wide mb-2">서브넷</h3>

...
<div class="bg-surface-base border border-line rounded-xl p-6 mb-6">
	...
	<p class="text-xs text-ink-2 mb-4">프로젝트별 개별 설정이 없을 때 적용되는 기본값입니다. 미설정 시 0 (GPU VM 생성 불가).</p>
	...
	{:else if allGpuTypes.length === 0}
		<div class="text-ink-2 text-sm">GPU alias를 찾을 수 없습니다. GPU flavor가 등록되어 있는지 확인하세요.</div>
	{:else}
		...
		<p class="text-xs text-ink-2 mt-2">-1 = 무제한, 0 = 사용 불가</p>
	{/if}
</div>

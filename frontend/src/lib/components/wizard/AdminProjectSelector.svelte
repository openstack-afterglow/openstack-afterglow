...
<div class="flex items-start justify-between mb-6">
	<div>
		...
		<p class="text-sm text-ink-2 mt-0.5">대상 프로젝트 선택</p>
	</div>
...
{:else if s.adminProjects.length === 0}
	<div class="text-center py-10 text-ink-2 text-sm">프로젝트 목록을 불러올 수 없습니다.</div>
{:else}
	...
	{#if s.filteredAdminProjects.length === 0}
		<div class="text-center py-10 text-ink-2 text-sm">검색 결과가 없습니다.</div>
	{:else}
				...
				>
					<div class="flex items-start justify-between gap-3">
						...
						{#if s.adminProjectQuotas.size === 0}
							<div class="text-ink-2 text-xs flex-shrink-0">quota 로딩 중...</div>
						{:else if q}
							<div class="flex flex-wrap items-center gap-1.5 flex-shrink-0 justify-end">
								<span class="px-1.5 py-0.5 rounded text-xs font-mono {s.isExhausted(q.instances) ? 'bg-red-900/40 text-red-300 border border-red-800/50' : 'bg-surface-base text-ink-2 border border-line-2'}" title="남은 인스턴스 (사용 {q.instances?.used ?? 0} / {q.instances?.quota < 0 ? '∞' : q.instances?.quota})">
									VM {s.fmtRemaining(q.instances)}
								</span>
								<span class="px-1.5 py-0.5 rounded text-xs font-mono {s.isExhausted(q.cpu) ? 'bg-red-900/40 text-red-300 border border-red-800/50' : 'bg-surface-base text-ink-2 border border-line-2'}" title="남은 vCPU (사용 {q.cpu?.used ?? 0} / {q.cpu?.quota < 0 ? '∞' : q.cpu?.quota})">
									CPU {s.fmtRemaining(q.cpu)}
								</span>
								<span class="px-1.5 py-0.5 rounded text-xs font-mono {s.isExhausted(q.ram_mb) ? 'bg-red-900/40 text-red-300 border border-red-800/50' : 'bg-surface-base text-ink-2 border border-line-2'}" title="남은 RAM MB (사용 {q.ram_mb?.used ?? 0} / {q.ram_mb?.quota < 0 ? '∞' : q.ram_mb?.quota})">
									RAM {q.ram_mb && q.ram_mb.quota >= 0 ? Math.max(0, Math.floor((q.ram_mb.quota - q.ram_mb.used) / 1024)) + 'GB' : '∞'}
								</span>
								<span class="px-1.5 py-0.5 rounded text-xs font-mono {s.isExhausted(q.disk_gb) ? 'bg-red-900/40 text-red-300 border border-red-800/50' : 'bg-surface-base text-ink-2 border border-line-2'}" title="남은 디스크 GB (사용 {q.disk_gb?.used ?? 0} / {q.disk_gb?.quota < 0 ? '∞' : q.disk_gb?.quota})">
									DISK {s.fmtRemaining(q.disk_gb)}{q.disk_gb && q.disk_gb.quota >= 0 ? 'GB' : ''}
								</span>
							</div>
						{:else}
							<div class="text-ink-2 text-xs flex-shrink-0">quota 없음</div>
						{/if}
					</div>
				</button>
			{/each}

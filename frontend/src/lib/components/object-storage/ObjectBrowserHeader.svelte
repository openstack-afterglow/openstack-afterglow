...
<div class="flex items-center gap-1 mb-2 flex-wrap text-sm">
	<a href={backHref} class="text-ink-2 hover:text-ink-2 flex items-center gap-1">
...
<div class="text-xs text-ink-2 uppercase tracking-widest mb-3">OBJECT STORAGE / EXPLORER</div>

<div class="flex items-start justify-between mb-4">
	<div>
		<h1 class="text-2xl font-bold text-ink-0">{s.containerName}</h1>
		{#if s.containerMeta}<p class="text-ink-2 text-xs mt-1">데이터 동기화 완료</p>{/if}
	</div>
	{#if s.containerMeta}
		<div class="flex gap-4 text-center">
			<div class="bg-surface-base border border-line rounded-lg px-4 py-2">
				<div class="text-xs text-ink-2 uppercase tracking-wider">총 용량</div>
				<div class="text-lg font-bold text-ink-0">
					...
					<span class="text-xs text-ink-2 font-normal">
						{s.containerMeta.bytes >= 1073741824 ? 'GB' : s.containerMeta.bytes >= 1048576 ? 'MB' : 'KB'}
					</span>
				</div>
			</div>
			<div class="bg-surface-base border border-line rounded-lg px-4 py-2">
				<div class="text-xs text-ink-2 uppercase tracking-wider">객체 수</div>
				<div class="text-lg font-bold text-ink-0">{s.containerMeta.count} <span class="text-xs text-ink-2 font-normal">Items</span></div>
			</div>
		</div>
	{/if}
</div>
...
<div class="flex items-center gap-1 mb-4 text-sm">
	...
	{#each s.breadcrumbs as seg, i}
		<svg class="w-3 h-3 text-ink-2 shrink-0" viewBox="0 0 20 20" fill="currentColor">

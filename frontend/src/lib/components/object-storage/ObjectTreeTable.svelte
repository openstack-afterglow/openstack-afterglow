...
{:else if s.treeRows.length === 0}
	<div class="text-ink-3 text-sm py-8 text-center">
		...
		{#if s.filterText.trim()}
			<p>검색 결과가 없습니다</p>
			<p class="text-ink-3 text-xs mt-1">다른 키워드를 입력하거나 검색 범위를 넓혀보세요</p>
		{:else}
			<p>오브젝트가 없습니다</p>
							...
							<div class="flex items-center gap-2.5" style="padding-left: {row.depth * 16}px">
								{#if isDir && !row.fullPath && (!s.filterText.trim() || s.searchScope === 'expanded')}
									<button
										...
										class="w-4 h-4 shrink-0 flex items-center justify-center text-ink-3 hover:text-ink-1 transition-colors"
									>
										{#if row.isLoading}

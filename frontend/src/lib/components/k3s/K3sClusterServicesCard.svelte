...
<div class="bg-surface-base border border-line rounded-lg p-5 mb-4">
	...
	{:else if s.services.length === 0}
		<div class="mt-4 text-sm text-ink-2 text-center py-6">Service 없음</div>
	{:else}
			...
			<table class="w-full text-xs">
				<thead>
					<tr class="border-b border-line">
						...
						<th class="text-left pb-2 text-xs uppercase tracking-wide text-ink-2 font-medium">타입</th>
						<th class="text-left pb-2 text-xs uppercase tracking-wide text-ink-2 font-medium">Cluster IP</th>
						<th class="text-left pb-2 text-xs uppercase tracking-wide text-ink-2 font-medium">포트</th>
						<th class="text-left pb-2 text-xs uppercase tracking-wide text-ink-2 font-medium">생성일</th>
						<th class="pb-2"></th>
					</tr>
				</thead>
				<tbody>
					{#each s.services as svc}
						...
						<tr class="border-b border-line/50 hover:bg-surface-sunken/30 transition-colors">
							...
							<td class="py-2.5 text-ink-2">{svc.created_at ? svc.created_at.slice(0, 10) : '—'}</td>

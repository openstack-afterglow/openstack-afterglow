...
<Modal open={true} onClose={onClose} labelledBy="instance-password-title">
	<div class="bg-surface-base border border-line-2 rounded-xl p-6 w-full max-w-md mx-4 shadow-[var(--shadow-restraint)]">
		...
		{#if s.passwordPrecheck?.os_admin_user}
			<p class="text-xs text-ink-2 mb-4">대상 계정: <span class="text-warm-text">{s.passwordPrecheck.os_admin_user}</span> (이미지 메타 기준)</p>
		{:else}
			<p class="text-xs text-ink-2 mb-4">대상 계정: 이미지 메타데이터의 <code class="text-warm-text">os_admin_user</code>로 자동 결정</p>
		{/if}

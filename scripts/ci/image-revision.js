...
//                                        ::error:: 를 쓰고 exit 1.
//   guard  <image-ref> <sha> <repo>    : 현재 발행된 ref 의 revision 이 sha 보다 앞서 있으면(compare=behind)
//                                        exit 3(retag 건너뜀). 알 수 없으면 exit 0(진행).
//
 ...
 * label 이 없거나 형식이 SHA 가 아니면 null. 플랫폼마다 revision 이 다르면 예외.
 */
...
/** 발행된 ref 의 revision. 이미지·label 이 없거나 조회에 실패하면 null(=알 수 없음). */
function readRevision(imageRef, exec = defaultExec) {
	try {
		return parseRevisionLabel(exec("docker", ["buildx", "imagetools", "inspect", imageRef, "--format", "{{json .Image}}"]));
	} catch {
		return null;
	}
}
...
/**
 * stale re-run 가드 결정. 현재 발행된 revision 이 이번 sha 보다 앞서 있을 때(compare base=published,
 * head=sha 가 behind)만 retag 를 건너뛴다. revision 을 모르거나 compare 가 실패하면 진행한다.
 */
function guardDecision({ imageRef, sha, repository }, exec = defaultExec) {
	const published = readRevision(imageRef, exec);
	if (!published) return { retag: true, reason: `${imageRef}: published revision unknown; proceeding` };
	if (published === sha) return { retag: true, reason: `${imageRef}: already published from ${sha}` };
	let status;
	try {
		status = compareStatus(repository, published, sha, exec);
	} catch (error) {
		return { retag: true, reason: `${imageRef}: compare ${published}...${sha} failed (${error.message}); proceeding` };
	}
	if (status === "behind") {
		return {
			retag: false,
			reason: `${imageRef} is published from ${published}, which is newer than ${sha}; skipping retag of a stale re-run`,
		};
	}
	return { retag: true, reason: `${imageRef}: ${sha} is ${status} relative to published ${published}` };
}
...
function main(argv, { exec = defaultExec, stdout = process.stdout, stderr = process.stderr } = {}) {
	...
	if (command === "guard") {
		const [imageRef, sha, repository] = rest;
		if (!imageRef || !SHA_RE.test(sha ?? "") || !repository) {
			stderr.write("usage: image-revision.js guard <image-ref> <40-hex sha> <owner/repo>\n");
			return 2;
		}
		const decision = guardDecision({ imageRef, sha, repository }, exec);
		if (!decision.retag) {

...
Operational status uses exact, case-sensitive `getStatusStyle(status: string | null | undefined)` and its five tones; unknown or falsy status is neutral. Status conveys label plus tone and, where applicable, transition dot; color alone is never state. Charts use the six ordinal `CHART_COLORS`; topology preserves its domain colors. The topology canvas view alternates tenant zones between `--color-topology-internal` and `--color-topology-internal-2`, uses `--color-topology-gateway` only for L3 device stripes and glyphs (never as a status tone), and paints zone fills with `color-mix()` at `--topology-zone-fill-alpha` (`--topology-zone-fill-alpha-active` when selected) over the `--color-topology-grid-minor`/`--color-topology-grid-major` grid. Topology trunk badges name their aggregation scope in visible text: a lower tenant switch link says `네트워크 합산`, while an external or shared provider-tier uplink says `하위망 합산` because it sums only the tenant networks directly behind that router rather than repeating or including provider-wide totals. The uplink badge renders only when it carries information the tenant badge does not — at least two tenant networks behind that router, and at most one badge per router — because a router with a single tenant network would repeat that network's own number, and the badge sits at the router↔switch midpoint, which is not required to fall inside its own zone.
